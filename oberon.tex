...
\figure{pdfs/fig-4.8.png}{4.7in}{4.8}{A pattern and its encoding as an array of bytes (in hex)}
...
\figure{pdfs/fig-4.9.png}{4.7in}{4.9}{General, regular cell structure of display area}
...
TODO: Figure 11.2 Path of messages to and from mailbox
...
In order to understand the process of compilation, the reader needs to be familiar with the source language Oberon defined in Appendix 1, and with the target computer \acro{RISC}, defined in Appendix 2.
...
It follows that we distinguish between two kinds of actions: analyzing steps and code generating steps. In a rough approximation we may say that the former are source language dependent and target computer independent, whereas the latter are source language independent and target computer dependent. Although reality is somewhat more complex, the module structure of this compiler clearly reflects this division. The main module of the compiler is \code{ORP} (for Oberon to \acro{RISC} Parser) It is primarily dedicated to syntactic analysis, parsing. Upon recognition of a syntactic construct, an appropriate procedure is called the code generator module \code{ORG} (for Oberon to \acro{RISC} Generator). Apart from parsing, \code{ORP} checks for type consistency of operands, and it computes the attributes of objects identified in declarations.
...
The amount of storage needed for a variable (called its size) is determined by the variable's type. The sizes of basic types are prescribed by the target computer's data representation. The following holds for the \acro{RISC} processor:
...
If the reference is to an element of a multi-dimensional array (matrix), its address computation involves several multiplications and additions. The address of an element $A[ik-1, ... , i1, i0]$ of a $k$-dimensional array $A$ with lengths $nk-1, ... , n1, n0$ is
